...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SessionRow(props) {
      ...
      /** 分叉（与官方树同一入口；fork 出来的会话继承同一个 cwd，所以仍是临时会话）。 */
      const fork = () => {
        setMenu(null)
        const uiWorkspace = serviceOf(ctx, 'uiWorkspace')
        if (!uiWorkspace || typeof uiWorkspace.forkSession !== 'function') return
        Promise.resolve(uiWorkspace.forkSession(sessionId)).catch((error) => {
          console.warn('dsh-project-context: fork failed:', error)
        })
      }
      ...
      return React.createElement('div', {
        ...
        React.createElement('div', {
          ...
          onMouseEnter: (event) => {
            setHover(true)
            setCard({ x: event.clientX + 12, y: event.clientY + 12 })
          },
          onMouseLeave: () => {
            setHover(false)
            setCard(null)
          ...
          hover && draft === null
            ? React.createElement('span', {
              ...
              iconAction('fork', d.actionFork, '⑂', fork),
        ...
        card === null || menu !== null || draft !== null ? null : React.createElement('div', {
          key: 'card',
          style: {
            position: 'fixed',
            left: card.x,
          ...
          React.createElement('span', {
            key: 'title',
            style: { fontSize: 14, lineHeight: '20px', color: 'var(--dsw-alias-label-primary, inherit)' },
          }, titleOf(session)),
          React.createElement('span', {
            key: 'path',
            style: {
          ...
          React.createElement('span', {
            key: 'time',
            style: { fontSize: 12, lineHeight: '16px', color: 'var(--dsw-alias-label-tertiary, #8a8a8e)' },
          }, relativeTime(session.updatedAt)),
        ]),
        menu === null ? null : React.createElement('div', {
          ...
          menuItem('fork', d.actionFork, fork),
        ...
        for (const group of groupByDay(rows)) {
          ...
          body.push(React.createElement('div', {
            ...
            React.createElement('div', {
              key: 'label',
              role: 'button',
              'data-workspace-purpose': 'group-header',
              ...
              React.createElement('span', { key: 'text', style: { flex: 1, minWidth: 0 } }, group.label),
              React.createElement('span', { key: 'count' }, String(group.rows.length)),
            ]),
            ...(isCollapsed ? [] : group.rows.map((session) => React.createElement(SessionRow, {
