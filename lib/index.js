...
/**
 * 临时会话工作区的状态块：说明它是临时沙箱，项目级约定不适用。
 * @param {string} cwd
 */
export function conversationText(cwd) {
  return [
    '<workspace_purpose>',
...
export function renderPurposeText(purpose, cwd) {
  ...
  if (!dir) return ''
  return purpose === 'conversation' ? conversationText(dir) : projectText(dir)
}
...
export function isConversationSession(session, registry) {
  ...
  if (!cwd) return false
  return normalizePathForCompare(cwd) === normalizePathForCompare(conversationDir())
}
