...
//      与技能目录的 digest 去重同款）。
//   4. 每会话单独开关（默认开启）：开关状态以 `~/.dsh/storages/project-context.json`
//      （`{ modes: { "<sessionId>": true|false } }`）文件存储，原子写（tmp+rename）。
//      webServer 提供 GET/POST /__project-context/state 供浏览器侧开关按钮调用。
//      历史：v0.3.x 曾用 session log 事件 `project-context/mode` 存储——但 harness
//      的已知事件类型白名单（KNOWN_SESSION_EVENT_TYPES）不包含仓库外插件的自定义
//      事件，且事件未被标记 ignorable 时，重启后恢复会话会被拒读
//      （SessionFormatUnsupportedError）。v0.4.0 起改用文件存储，不再写任何自定义
//      会话事件；旧版本已写入的事件仅作为**读取回退**（fold 最近一条），绝不新增。
//   5. 更早版本 plain-object 格式的 `project-context.json`（`{disabled:{...}}`）在
//      启动时一次性迁移进新结构（幂等），随后原文件改名 .migrated 留存备份。
//
...
// --- 状态：~/.dsh/storages/project-context.json（文件存储，默认开启） ----------
// 文件结构：`{ "modes": { "<sessionId>": true|false } }`。
// 生效值优先级：文件 → 会话旧事件（fold，兼容 v0.3.x 已写入的 mode 事件）→
// 默认开启（无事件即此值）。
// 与 `sandbox/mode` 的"log 即存储"不同：本项目选择文件存储，是因为 harness
// 的已知事件类型白名单不含仓库外插件事件（读回退兼容不影响此决策）。
...
// 读文件里的 modes 映射；文件缺失/损坏/非新结构时返回 null（区别于空对象）。
// 只接受布尔值条目；其余视为脏数据丢弃。
export function readModesFile() {
  let raw
  try {
  ...
  const m = raw && typeof raw === 'object' && raw.modes && typeof raw.modes === 'object' ? raw.modes : null
  if (!m) return null
  const out = {}
...
// 原子写 modes 映射（tmp + rename；Windows 上 rename 覆盖已存在文件）。
export function writeModesFile(modes) {
  const file = stateFile()
  const dir = path.dirname(file)
  try {
    fs.mkdirSync(dir, { recursive: true })
  } catch { /* 目录已存在或不可建 → 让写失败自然暴露 */ }
  const tmp = file + '.tmp'
  try {
    fs.writeFileSync(tmp, JSON.stringify({ modes }, null, 2), 'utf8')
...
// 模块级内存态：norm(sessionId) → enabled。会话挂载时从文件加载，切换时同步写回。
const fileModes = new Map()

// 从文件重新加载内存态（先清空再填充；文件缺失则保持空）。
export function loadModes() {
  fileModes.clear()
  const m = readModesFile()
  if (!m) return
  for (const [k, v] of Object.entries(m)) fileModes.set(norm(k), v)
}

// 把当前内存态原子写回文件。
export function persistModes() {
  writeModesFile(Object.fromEntries(fileModes))
}

// 兼容取会话事件数组：真实 Session 已无 `events` 成员（新版 harness 以
// `snapshotEvents()` / `eventAt(seq)` 取代），先取 `snapshotEvents()`；旧的 plain-object
// 结构（含单元测试的假 session）仍可直接用 `events` 数组。两者都取不到时返回 undefined。
...
// 折叠会话事件：返回最近一条 project-context/mode 的 enabled，无事件时 undefined。
// （保留为纯函数：v0.3.x 历史事件的读取回退 + 单元测试；新会话不再写入此类事件。）
...
// 当前生效值：文件优先 → 会话旧事件（fold）→ undefined（默认开启）。
export function resolveMode(session) {
  const sid = norm(session && session.id)
  if (sid && fileModes.has(sid)) return fileModes.get(sid)
  return projectModeOf(session)
}

// 旧版 plain-object 文件迁移：`{disabled:{id:true}}` → 新结构 modes 条目
// （不写会话事件，直接落文件），然后改名备份。幂等：已迁移（存在 .migrated、
// 已是新结构、或 modes 已有该 id）则不重复写。
function migrateLegacyJson() {
  ...
  if (raw && typeof raw === 'object' && raw.modes && typeof raw.modes === 'object') return // 已是新版
  const disabledMap =
    raw && typeof raw === 'object' && raw.disabled && typeof raw.disabled === 'object'
      ? raw.disabled
      : {}
  const ids = Object.keys(disabledMap).filter((k) => disabledMap[k] === true)
  if (ids.length === 0) return
  const modes = readModesFile() || {}
  let migrated = 0
  for (const sid of ids) {
    const wanted = norm(sid)
    if (!wanted || modes[wanted] !== undefined) continue
    modes[wanted] = false
    migrated += 1
  }
  if (migrated === 0) return
  try {
    writeModesFile(modes)
  } catch { /* 写失败不阻塞启动 */ }
  try {
...
// 事件数组来自 sessionEvents()——真实 Session 用 snapshotEvents()，兼容旧 events 结构。
export function projectHistoryVisibleText(session) {
  const events = sessionEvents(session)
  if (!Array.isArray(events) || !session.surface || !session.surface.nodes) return undefined
...
// fileMode 显式传入时优先（供单元测试注入，不触文件）；缺省走 resolveMode(session)。
export function renderProjectText(session, fileMode) {
  const cwd = cwdOf(session)
  if (!cwd) return ''
  const mode = fileMode !== undefined ? fileMode : resolveMode(session)
  return mode === false ? projectDisabledText(cwd) : projectContextText(cwd)
}
...
// 查询某会话的开关状态：tracked（是否为有 cwd 的工作区项目）+ enabled（生效值）。
function getSessionState(ctx, sessionId) {
  ...
  const disabled = tracked && resolveMode(session) === false
  return { tracked, cwd, enabled: tracked && !disabled, disabled }
}

// --- plugin ----------------------------------------------------------------
function apply(ctx) {
  // 加载文件状态；迁移旧版 plain-object json（sessions 服务就绪后再执行——
  // 迁移只操作文件，不再依赖 sessions 服务，早执行亦可，此处保留时序防文件竞争）。
  loadModes()
  ctx.inject(['sessions'], () => migrateLegacyJson())
    ...
    targetCtx.effect(() => host.register({
      ...
      handler: async (req, res) => {
        ...
        if (req.method === 'POST') {
          ...
          // 写路径：更新内存态并原子落盘（不再写会话事件——自定义事件类型不
          // 在 harness 已知白名单内，写日志会污染"重放即状态"的会话文件）。下次
          // 模型步骤快照内容变化 → 模型立即看到"降级为普通工作区"（或恢复约定）。
          const sid = norm(sessionId)
          fileModes.set(sid, enabled)
          try {
            persistModes()
          } catch {
            fileModes.set(sid, resolveMode(session))
            sendJson(res, 500, { ok: false, error: 'failed to persist mode state' })
            return
          }
          sendJson(res, 200, { ok: true, ...getSessionState(ctx, sessionId) })
          return
