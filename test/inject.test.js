...
test('renderPurposeText: 项目会话给 project_context，临时会话什么都不给', () => {
  assert.equal(renderPurposeText('project', 'C:/a'), projectText('C:/a'))
  assert.equal(renderPurposeText('conversation', 'C:/a'), '')
})
...
test('isConversationSession: 临时目录之下的子目录也算（兼容 0.6–0.8 的每会话子目录）', () => {
  const registry = { get: () => undefined }
  assert.equal(isConversationSession(session({ cwd: path.join(CONV_DIR, 'abc') }), registry), true)
  assert.equal(isConversationSession(session({ cwd: path.join(CONV_DIR, 'a', 'b') }), registry), true)
})

test('isConversationSession: 相似前缀不算（default-other 不是 default 的子目录）', () => {
  const registry = { get: () => undefined }
  assert.equal(isConversationSession(session({ cwd: `${CONV_DIR}-other` }), registry), false)
})
